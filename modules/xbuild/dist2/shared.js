...
//#endregion
//#region ../common/src/object/index.ts
function isObject(value) {
  return typeof value === "object" && value !== null;
}
//#endregion
//#region ../common/src/assert/index.ts
...
//#endregion
//#region ../common/src/number/index.ts
...
//#endregion
//#region ../common/src/collection/index.ts
var AnyMap = class {
  originMap;
  objectMap;
  constructor() {
    this.originMap = /* @__PURE__ */new Map();
...
var Collection = class {
  ...
  */
  addKey(key, element, force) {
    ...
    if (index === -1) {
      index = this.elements.push(element) - 1;
      this.indexMap.set(key, index);
    } else if (force) this.elements.splice(index, 1, element);
  }
  /**
  * 批量添加元素
  ...
  */
  remove(key) {
    if (this.indexMap.has(key)) {
      ...
      for (let i = index; i < this.elements.length; i++) this.indexMap.set(this.getKey(this.elements[i]), i);
      return removed;
    }
  }
  /**
  ...
  */
  each(handler) {
    [...this.elements].forEach((element, index) => handler(element, this.getKey(element), index, this));
  }
...
//#endregion
//#region ../common/src/permission/index.ts
const createPermission = (permissionState) => {
  return class Permission {
    permission;
    permissionState;
    static from(flag) {
      if (typeof flag === "number") return new this(flag);
      if (flag instanceof this) return flag;
    ...
    parsePermissions(permissions, force) {
      const keys = new Set(Reflect.ownKeys(this.permissionState));
      const res = [];
      if (permissions.length === 0 && force) return Array.from(keys);
      for (const permission of permissions) {
        if (!keys.has(permission)) throw TypeError("unknown permission: \"" + Object(permission).toString + "\"");
        res.push(permission);
      }
      return res;
    }
    has(...permissions) {
      ...
      return permissions.every((permission) => this.permission & this.permissionState[permission]);
    }
    add(...permissions) {
      permissions = this.parsePermissions(permissions);
      this.permission |= permissions.reduce((acc, permission) => acc | this.permissionState[permission], 0);
      return this;
    }
    remove(...permissions) {
      permissions = this.parsePermissions(permissions);
...
//#endregion
//#region ../common/src/global/index.ts
...
const PropertyPermission = createPermission({
  enumerable: 4,
  writable: 2,
  configurable: 1
});
...
* @param value
*/
function defineProperty(target, propKey, permission = 7, value) {
...
function tryCall(call, data, receiver) {
  if (typeof call === "function") try {
    return Reflect.apply(call, receiver, data || []);
  } catch (e) {
    throw e instanceof Error ? e : new Error(e);
  } else
  throw `${typeof call === "object" ? Reflect.get(call, "name", call) : Object(call).toString()} is not a function.`;
}
//#endregion
//#region ../common/src/array/index.ts
function isArray(o) {
  return Array.isArray(o);
}
setGlobalData("@msom/component", {
  componentDefinitionKey: Symbol("component_definition"),
  componentMap: /* @__PURE__ */new Map()
});
//#endregion
//#region ../common/src/nil/index.ts
function nil(data, defaultData) {
  if (data === void 0 || data === null) return defaultData;
  return data;
}
//#endregion
//#region ../common/src/overload/createOverload.ts
const TYPE_SPLITOR = ",";
...
//#endregion
//#region ../common/src/promise/OcPromise/types.ts
const PENDDING = "pendding";
const FULFILLED = "fulfilled";
const REJECTED = "rejected";
const CANCELED = "canceled";
//#endregion
//#region ../../node_modules/.pnpm/uuidv7@1.1.0/node_modules/uuidv7/dist/index.js
/**
...
var UUID = class UUID {
  ...
  */
  static ofInner(bytes) {
    if (bytes.length !== 16) throw new TypeError("not 128-bit length");else
  ...
  static parse(uuid) {
    ...
    switch (uuid.length) {
      ...
      case 38:
        ...
        break;
      case 45:hex = (_d = /^urn:uuid:([0-9a-f]{8})-([0-9a-f]{4})-([0-9a-f]{4})-([0-9a-f]{4})-([0-9a-f]{12})$/i.exec(uuid)) === null || _d === void 0 ? void 0 : _d.slice(1, 6).join("");
    }
    if (hex) {
  ...
  */
  getVariant() {
    ...
    if (n < 0) throw new Error("unreachable");else
    if (n <= 7) return this.bytes.every((e) => e === 0) ? "NIL" : "VAR_0";else
    if (n <= 11) return "VAR_10";else
    if (n <= 13) return "VAR_110";else
    if (n <= 15) return this.bytes.every((e) => e === 255) ? "MAX" : "VAR_RESERVED";else
...
var V7Generator = class {
  ...
  */
  generateOrAbortCore(unixTsMs, rollbackAllowance) {
    ...
    if (!Number.isInteger(unixTsMs) || unixTsMs < 0 || unixTsMs > 0xffffffffffff) throw new RangeError("`unixTsMs` must be a 48-bit unsigned integer");else
...
const getDefaultRandom = () => {
  if (typeof crypto !== "undefined" && typeof crypto.getRandomValues !== "undefined") return new BufferedCryptoRandom();else
  {
...
//#endregion
//#region ../common/src/promise/nextTick/index.ts
/**
* 执行延迟调用，并返回一个唯一ID
* @param task 延迟执行的函数
* @returns 唯一ID
*/
const nextTickStore = { tickMap: /* @__PURE__ */new Map() };
/**
* 执行延迟调用
* @param task 延迟执行的函数
* @returns 唯一ID
...
const nextTick = (task) => {
  ...
  if (typeof process !== "undefined" && process.nextTick) {
    ...
    const dom = document.createTextNode(currentId);
    nextTickStore.tickMap.set(currentId, option);
    ob.observe(dom, { characterData: true });
    dom.data = currentId + "1";
    option.cancel = () => {
      _option.canceled = true;
      option.cancel = void 0;
      cleanup();
...
//#endregion
//#region ../common/src/promise/OcPromise/OcPromiseCanceledException.ts
var OcPromiseCanceledException = class extends Error {
  constructor(reason) {
    super(`OcPromise Canceled: ${reason}`);
    this.name = "OcPromiseCanceledException";
  }
};
const isOcPromiseCanceledException = (e) => e instanceof OcPromiseCanceledException;
const formatOcPromiseCanceledException = (e) => {
  if (isOcPromiseCanceledException(e)) return e;
  return new OcPromiseCanceledException(e);
};
//#endregion
//#region ../common/src/promise/OcPromise/utils.ts
...
//#endregion
//#region ../common/src/promise/OcPromise/OcPromise.ts
/**
...
var OcPromise = class OcPromise {
  ...
  */
  constructor(executor) {
    ...
    const cancel = (reason) => {
      this.changeStatus(CANCELED, formatOcPromiseCanceledException(reason));
    };
    try {
  ...
  */
  _runThens() {
    ...
    while (this.handlers.length) {
      ...
      const { resolve, cancel, onFulfilled, onRejected, onCanceled } = handler;
      let { reject } = handler;
      reject = (e) => {
        if (e instanceof OcPromiseCanceledException) cancel(e);else
        reject(e);
      };
      ...
      const toggleAwait = () => {
        if (this.status === "canceled" && onRejected) try {
      ...
      const task = () => {
        try {
          const data = exe();
          toggleAwait();
          if (isOcPromiseLike(data)) nextTick(() => {
  ...
  */
  cancel(reason, cascade = true) {
    if (this.parrent && this.parrent.status === "pendding" && cascade) this.parrent.cancel(reason, cascade);else
    this.changeStatus(CANCELED, formatOcPromiseCanceledException(reason));
  }
  /**
  * 等待所有 Promise 完成
  ...
  static all(promiseables) {
    ...
    return new OcPromise((resolve, reject, cancel) => {
      ...
      while (!next.done) {
        ...
        promiseCount++;
        const { value } = next;
        if (isOcPromise(value)) value.then((data) => singlePromiseResolver(data, promiseIndex), reject, cancel);else
        if (isPromiseLike(value)) value.then((data) => singlePromiseResolver(data, promiseIndex), reject);else
        {
  ...
  */
  static withResolvers() {
    const resolvers = {};
    return {
      promise: new OcPromise((resolve, reject, cancel) => {
  ...
  */
  static resolve(value) {
    ...
    if (isPromiseLike(value)) return new OcPromise((resolve, reject, cancel) => {
      value.then(resolve, (e) => {
        if (isOcPromiseCanceledException(e)) cancel(e);else
        reject(e);
      });
    });
...
OcPromise.resolve("S").then(null, (data) => {
  return new Promise(() => {});
}).then((data) => {});
Promise.resolve("S").then(null, () => {
  return new Promise(() => {});
}).then((data) => {});
new class Client {
  config;
  ...
  applyParams(url, params) {
    ...
    if (params instanceof URLSearchParams) normalizedParams = params;else
    if (Array.isArray(params)) normalizedParams = new URLSearchParams(params);else
    if (typeof params === "object" && params !== null) normalizedParams = new URLSearchParams(Object.entries(params));else
    ...
    try {
      if (typeof url === "string") {
        const baseURL = url.startsWith("/") ? this.config.baseURL || window.location.origin : void 0;
  ...
  request(url, init = {}) {
    ...
    if (init.headers) Object.entries(init.headers).forEach(([key, value]) => {
    ...
    const executeRequestInterceptors = (currentPromise) => {
      if (this.config.requestInterceptors) for (const interceptor of this.config.requestInterceptors) currentPromise = currentPromise.then((res) => {
        return OcPromise.resolve(interceptor(...res));
      });
      return currentPromise;
    };
    const executeResponseInterceptors = (promise) => {
      let currentPromise = promise;
      if (this.config.responseInterceptors) for (const interceptor of this.config.responseInterceptors) currentPromise = currentPromise.then((res) => {
        return OcPromise.resolve(interceptor(res));
      });
      return currentPromise;
    };
    const initPromise = OcPromise.resolve([processedUrl, fetchInit]);
    let fetchPromise;
  ...
  */
  requestJson(url, init) {
    return this.json(this.request(url, init));
  }
  /**
  ...
  * @returns 响应Promise
  */
  jsonRequestJson(url, init) {
    return this.json(this.jsonRequest(url, init));
  }
}();
//#endregion
  ...
  (function () {
    ...
    function isOriginAllowed(origin, allowedOrigin) {
      if (Array.isArray(allowedOrigin)) {
        ...
        return false;
      } else if (isString(allowedOrigin)) return origin === allowedOrigin;else
      if (allowedOrigin instanceof RegExp) return allowedOrigin.test(origin);else
    ...
    function applyHeaders(headers, res) {
      for (var i = 0, n = headers.length; i < n; i++) {
        ...
        if (header) {
          if (Array.isArray(header)) applyHeaders(header, res);else
          if (header.key === "Vary" && header.value) vary(res, header.value);else
          if (header.value) res.setHeader(header.key, header.value);
        }
      }
    }
    function cors(options, req, res, next) {
      ...
      if ((req.method && req.method.toUpperCase && req.method.toUpperCase()) === "OPTIONS") {
        ...
        if (options.preflightContinue) next();else
        {
    ...
    function middlewareWrapper(o) {
      ...
      if (typeof o === "function") optionsCallback = o;else
      ...
      return function corsMiddleware(req, res, next) {
        optionsCallback(req, function (err, options) {
          if (err) next(err);else
          {
            ...
            if (corsOptions.origin && typeof corsOptions.origin === "function") originCallback = corsOptions.origin;else
            if (corsOptions.origin) originCallback = function (origin, cb) {
              cb(null, corsOptions.origin);
            };
            if (originCallback) originCallback(req.headers.origin, function (err2, origin) {
              if (err2 || !origin) next(err2);else
              {
...
var require_ms = /* @__PURE__ */__commonJSMin((exports, module) => {
  ...
  */
  module.exports = function (val, options) {
    options = options || {};
    var type = typeof val;
    if (type === "string" && val.length > 0) return parse(val);else
  ...
  function setup(env) {
    ...
    */
    function enable(namespaces) {
      ...
      for (const ns of split) if (ns[0] === "-") createDebug.skips.push(ns.slice(1));else
...
var require_browser$2 = /* @__PURE__ */__commonJSMin((exports, module) => {
  ...
  */
  function save(namespaces) {
    try {
      if (namespaces) exports.storage.setItem("debug", namespaces);else
  ...
  */
  function format(value, options) {
    ...
    if (!unit || !map[unit.toLowerCase()]) {
      if (mag >= map.pb) unit = "PB";else
      if (mag >= map.tb) unit = "TB";else
      if (mag >= map.gb) unit = "GB";else
      if (mag >= map.mb) unit = "MB";else
      if (mag >= map.kb) unit = "KB";else
      unit = "B";
    }
...
var require_safer = /* @__PURE__ */__commonJSMin((exports, module) => {
  ...
  if (!Safer.alloc) Safer.alloc = function (size, fill, encoding) {
    ...
    var buf = Buffer(size);
    if (!fill || fill.length === 0) buf.fill(0);else
    if (typeof encoding === "string") buf.fill(fill, encoding);else
    buf.fill(fill);
    return buf;
  };
...
var require_internal = /* @__PURE__ */__commonJSMin((exports, module) => {
  ...
  function InternalCodec(codecOptions, iconv) {
    ...
    if (this.enc === "base64") this.encoder = InternalEncoderBase64;else
    if (this.enc === "cesu8") {
      this.enc = "utf8";
  ...
  InternalEncoderCesu8.prototype.write = function (str) {
    ...
    for (var i = 0; i < str.length; i++) {
      ...
      if (charCode < 128) buf[bufIdx++] = charCode;else
      if (charCode < 2048) {
  ...
  InternalDecoderCesu8.prototype.write = function (buf) {
    ...
    for (var i = 0; i < buf.length; i++) {
      ...
      if ((curByte & 192) !== 128) {
        ...
        if (curByte < 128) res += String.fromCharCode(curByte);else
        if (curByte < 224) {
          acc = curByte & 31;
          contBytes = 1;
          accBytes = 1;
        ...
        accBytes++;
        if (contBytes === 0) {
          if (accBytes === 2 && acc < 128 && acc > 0) res += this.defaultCharUnicode;else
          if (accBytes === 3 && acc < 2048) res += this.defaultCharUnicode;else
...
var require_utf32 = /* @__PURE__ */__commonJSMin((exports) => {
  ...
  Utf32Encoder.prototype.end = function () {
    ...
    if (this.isLE) buf.writeUInt32LE(this.highSurrogate, 0);else
  ...
  Utf32Decoder.prototype.write = function (src) {
    ...
    if (overflow.length > 0) {
      ...
      if (overflow.length === 4) {
        if (isLE) codepoint = overflow[i] | overflow[i + 1] << 8 | overflow[i + 2] << 16 | overflow[i + 3] << 24;else
    ...
    for (; i < src.length - 3; i += 4) {
      if (isLE) codepoint = src[i] | src[i + 1] << 8 | src[i + 2] << 16 | src[i + 3] << 24;else
...
var require_utf7 = /* @__PURE__ */__commonJSMin((exports) => {
  ...
  Utf7Decoder.prototype.write = function (buf) {
    ...
    if (!inBase64) res += this.iconv.decode(buf.slice(lastI), "ascii");else
    {
...
var require_dbcs_codec = /* @__PURE__ */__commonJSMin((exports) => {
  ...
  function DBCSCodec(codecOptions, iconv) {
    ...
    if (typeof codecOptions.gb18030 === "function") {
      ...
      for (var i = 129; i <= 254; i++) {
        ...
        for (var j = 48; j <= 57; j++) {
          if (secondByteNode[j] === UNASSIGNED) secondByteNode[j] = NODE_START - commonThirdByteNodeIdx;else
          if (secondByteNode[j] > NODE_START) throw new Error("gb18030 decode tables conflict at byte 2");
          var thirdByteNode = this.decodeTables[NODE_START - secondByteNode[j]];
          for (var k = 129; k <= 254; k++) {
            if (thirdByteNode[k] === UNASSIGNED) thirdByteNode[k] = NODE_START - commonFourthByteNodeIdx;else
            if (thirdByteNode[k] === NODE_START - commonFourthByteNodeIdx) continue;else
            if (thirdByteNode[k] > NODE_START) throw new Error("gb18030 decode tables conflict at byte 3");
  ...
  DBCSCodec.prototype._getDecodeTrieNode = function (addr) {
    ...
    for (var i = bytes.length - 1; i > 0; i--) {
      ...
      } else if (val <= NODE_START) node = this.decodeTables[NODE_START - val];else
    ...
    for (var k = 1; k < chunk.length; k++) {
      ...
      if (typeof part === "string") for (var l = 0; l < part.length;) {
        ...
        if (55296 <= code && code < 56320) {
          ...
          if (56320 <= codeTrail && codeTrail < 57344) writeTable[curAddr++] = 65536 + (code - 55296) * 1024 + (codeTrail - 56320);else
  ...
  DBCSCodec.prototype._setEncodeChar = function (uCode, dbcsCode) {
    ...
    var low = uCode & 255;
    if (bucket[low] <= SEQ_START) this.encodeTableSeq[SEQ_START - bucket[low]][DEF_CHAR] = dbcsCode;else
    if (bucket[low] == UNASSIGNED) bucket[low] = dbcsCode;
  };
    ...
    for (var i = 0; i < 256; i++) {
      ...
      if (uCode >= 0) {
        ...
        if (!subNodeEmpty[subNodeIdx]) {
          ...
          if (this._fillEncodeTable(subNodeIdx, newPrefix, skipEncodeChars)) hasValues = true;else
    ...
    while (true) {
      ...
      if (seqObj !== void 0 && uCode != UNASSIGNED) {
        ...
        if (typeof resCode === "object") {
          seqObj = resCode;
          continue;
        } else if (typeof resCode == "number") dbcsCode = resCode;else
        if (resCode == void 0) {
      ...
      if (dbcsCode < 256) newBuf[j++] = dbcsCode;else
      if (dbcsCode < 65536) {
        newBuf[j++] = dbcsCode >> 8;
  ...
  DBCSDecoder.prototype.write = function (buf) {
    ...
    for (var i = 0, j = 0; i < buf.length; i++) {
      ...
      if (uCode >= 0) {} else if (uCode === UNASSIGNED) {
        ...
        if (i >= 3) var ptr = (buf[i - 3] - 129) * 12600 + (buf[i - 2] - 48) * 1260 + (buf[i - 1] - 129) * 10 + (curByte - 48);else
...
//#endregion
//#region (ignored) ../../node_modules/.pnpm/iconv-lite@0.6.3/node_modules/iconv-lite/lib
...
var require_lib$1 = /* @__PURE__ */__commonJSMin((exports, module) => {
  ...
  var stream_module;
  try {
    stream_module = require_lib$2();
  } catch (e) {}
  if (stream_module && stream_module.Transform) iconv.enableStreamingAPI(stream_module);else
...
var require_raw_body = /* @__PURE__ */__commonJSMin((exports, module) => {
  ...
  */
  ...
  var createError$5 = __require("http-errors");
  ...
  */
  function getRawBody(stream, options, callback) {
    var done = callback;
    var opts = options || {};
    if (stream === void 0) throw new TypeError("argument stream is required");else
    if (typeof stream !== "object" || stream === null || typeof stream.on !== "function") throw new TypeError("argument stream must be a stream");
  ...
  */
  function readStream(stream, encoding, length, limit, callback) {
    var complete = false;
    var sync = true;
    if (limit !== null && length !== null && length > limit) return done(createError$5(413, "request entity too large", {
      expected: length,
      length,
      limit,
    ...
    if (stream._decoder || state && (state.encoding || state.decoder)) return done(createError$5(500, "stream encoding should not be set", { type: "stream.encoding.set" }));
    if (typeof stream.readable !== "undefined" && !stream.readable) return done(createError$5(500, "stream is not readable", { type: "stream.not.readable" }));
    var received = 0;
    var decoder;
    try {
    ...
    function done() {
      ...
      for (var i = 0; i < args.length; i++) args[i] = arguments[i];
      complete = true;
      if (sync) process.nextTick(invokeCallback);else
      invokeCallback();
    ...
    function onData(chunk) {
      ...
      if (limit !== null && received > limit) done(createError$5(413, "request entity too large", {
        limit,
        received,
        type: "entity.too.large"
      }));else
      if (decoder) buffer += decoder.write(chunk);else
      buffer.push(chunk);
    }
    function onEnd(err) {
      if (complete) return;
...
//#endregion
//#region ../../node_modules/.pnpm/body-parser@2.2.0/node_modules/body-parser/lib/read.js
/*!
* body-parser
...
var require_read = /* @__PURE__ */__commonJSMin((exports, module) => {
  ...
  * @private
  */
  var createError$4 = __require("http-errors");
  ...
  function read(req, res, next, parse, debug, options) {
    ...
    getBody(stream, opts, function (error, body) {
      if (error) {
        ...
        _error = createError$4(400, error);
        if (stream !== req) {
          req.unpipe();
          stream.destroy();
        }
  ...
  */
  function dump(req, callback) {
    if (onFinished.isFinished(req)) callback(null);else
    {
...
var require_content_type = /* @__PURE__ */__commonJSMin((exports) => {
  ...
  */
  function getcontenttype(obj) {
    var header;
    if (typeof obj.getHeader === "function") header = obj.getHeader("content-type");else
    if (typeof obj.headers === "object") header = obj.headers && obj.headers["content-type"];
...
//#endregion
//#region ../../node_modules/.pnpm/mime-types@3.0.1/node_modules/mime-types/mimeScore.js
...
//#endregion
//#region ../../node_modules/.pnpm/mime-types@3.0.1/node_modules/mime-types/index.js
/*!
* mime-types
* Copyright(c) 2014 Jonathan Ong
* Copyright(c) 2015 Douglas Christopher Wilson
...
//#endregion
//#region ../../node_modules/.pnpm/type-is@2.0.1/node_modules/type-is/index.js
/*!
* type-is
* Copyright(c) 2014 Jonathan Ong
* Copyright(c) 2014-2015 Douglas Christopher Wilson
...
//#endregion
//#region ../../node_modules/.pnpm/body-parser@2.2.0/node_modules/body-parser/lib/utils.js
...
//#endregion
//#region ../../node_modules/.pnpm/body-parser@2.2.0/node_modules/body-parser/lib/types/json.js
/*!
* body-parser
* Copyright(c) 2014 Jonathan Ong
* Copyright(c) 2014-2015 Douglas Christopher Wilson
...
var require_json = /* @__PURE__ */__commonJSMin((exports, module) => {
  ...
  * @private
  */
  var createError$3 = __require("http-errors");
  var debug = require_browser$2()("body-parser:json");
  var isFinished = require_on_finished().isFinished;
  var read = require_read();
  var typeis = require_type_is();
  var { getCharset, normalizeOptions } = require_utils$2();
  /**
  * Module exports.
  */
  module.exports = json;
  ...
  function json(options) {
    ...
    return function jsonParser(req, res, next) {
      ...
      if (charset.slice(0, 4) !== "utf-") {
        ...
        next(createError$3(415, "unsupported charset \"" + charset.toUpperCase() + "\"", {
          charset,
          type: "charset.unsupported"
        }));
        return;
...
//#endregion
//#region ../../node_modules/.pnpm/body-parser@2.2.0/node_modules/body-parser/lib/types/raw.js
/*!
* body-parser
* Copyright(c) 2014-2015 Douglas Christopher Wilson
* MIT Licensed
...
var require_raw = /* @__PURE__ */__commonJSMin((exports, module) => {
  ...
  * Module dependencies.
  */
  var debug = require_browser$2()("body-parser:raw");
  var isFinished = require_on_finished().isFinished;
  var read = require_read();
  var typeis = require_type_is();
...
//#endregion
//#region ../../node_modules/.pnpm/body-parser@2.2.0/node_modules/body-parser/lib/types/text.js
/*!
* body-parser
* Copyright(c) 2014-2015 Douglas Christopher Wilson
* MIT Licensed
...
var require_text = /* @__PURE__ */__commonJSMin((exports, module) => {
  ...
  * Module dependencies.
  */
  var debug = require_browser$2()("body-parser:text");
  var isFinished = require_on_finished().isFinished;
  var read = require_read();
  var typeis = require_type_is();
  var { getCharset, normalizeOptions } = require_utils$2();
...
var require_object_inspect = /* @__PURE__ */__commonJSMin((exports, module) => {
  ...
  module.exports = function inspect_(obj, options, depth, seen) {
    ...
    if (typeof seen === "undefined") seen = [];else
    if (indexOf(seen, obj) >= 0) return "[Circular]";
    ...
    if (typeof obj === "object" && customInspect) {
      if (inspectSymbol && typeof obj[inspectSymbol] === "function" && utilInspect) return utilInspect(obj, { depth: maxDepth - depth });else
  ...
  function getIndent(opts, depth) {
    ...
    if (typeof opts.indent === "number" && opts.indent > 0) baseIndent = $join.call(Array(opts.indent + 1), " ");else
    return null;
    return {
      base: baseIndent,
  ...
  function arrObjKeys(obj, inspect) {
    ...
    for (var key in obj) {
      ...
      if (hasShammedSymbols && symMap["$" + key] instanceof Symbol) continue;else
      if ($test.call(/[^\w$]/, key)) xs.push(inspect(key, obj) + ": " + inspect(obj[key], obj));else
...
var require_get_intrinsic = /* @__PURE__ */__commonJSMin((exports, module) => {
  ...
  var doEval = function doEval(name) {
    var value;
    if (name === "%AsyncFunction%") value = getEvalledConstructor("async function () {}");else
    if (name === "%GeneratorFunction%") value = getEvalledConstructor("function* () {}");else
    if (name === "%AsyncGeneratorFunction%") value = getEvalledConstructor("async function* () {}");else
  ...
  var stringToPath = function stringToPath(string) {
    ...
    if (first === "%" && last !== "%") throw new $SyntaxError("invalid intrinsic syntax, expected closing `%`");else
  ...
  module.exports = function GetIntrinsic(name, allowMissing) {
    ...
    for (var i = 1, isOwn = true; i < parts.length; i += 1) {
      ...
      if (hasOwn(INTRINSICS, intrinsicRealName)) value = INTRINSICS[intrinsicRealName];else
      if (value != null) {
        if (!(part in value)) {
        ...
        if ($gOPD && i + 1 >= parts.length) {
          ...
          isOwn = !!desc;
          if (isOwn && "get" in desc && !("originalValue" in desc.get)) value = desc.get;else
          value = value[part];
        } else {
...
var require_utils$1 = /* @__PURE__ */__commonJSMin((exports, module) => {
  ...
  var merge = function merge(target, source, options) {
    ...
    if (isArray(target) && isArray(source)) {
      source.forEach(function (item, i) {
        if (has.call(target, i)) {
          ...
          if (targetItem && typeof targetItem === "object" && item && typeof item === "object") target[i] = merge(targetItem, item, options);else
          target.push(item);
        } else target[i] = item;
      });
      return target;
    ...
    return Object.keys(source).reduce(function (acc, key) {
      ...
      if (has.call(acc, key)) acc[key] = merge(acc[key], value, options);else
      acc[key] = value;
      return acc;
    }, mergeTarget);
  };
  ...
  module.exports = {
    ...
    encode: function encode(str, defaultEncoder, charset, kind, format) {
      ...
      var string = str;
      if (typeof str === "symbol") string = Symbol.prototype.toString.call(str);else
      if (typeof str !== "string") string = String(str);
...
var require_stringify = /* @__PURE__ */__commonJSMin((exports, module) => {
  ...
  var stringify = function stringify(object, prefix, generateArrayPrefix, commaRoundTrip, allowEmptyArrays, strictNullHandling, skipNulls, encodeDotInKeys, encoder, filter, sort, allowDots, serializeDate, format, formatter, encodeValuesOnly, charset, sideChannel) {
    ...
    while ((tmpSc = tmpSc.get(sentinel)) !== void 0 && !findFlag) {
      ...
      if (typeof pos !== "undefined") {
        if (pos === step) throw new RangeError("Cyclic object value");else
        findFlag = true;
      }
      if (typeof tmpSc.get(sentinel) === "undefined") step = 0;
    }
    if (typeof filter === "function") obj = filter(prefix, obj);else
    if (obj instanceof Date) obj = serializeDate(obj);else
    if (generateArrayPrefix === "comma" && isArray(obj)) obj = utils.maybeMap(obj, function (value) {
  ...
  var normalizeStringifyOptions = function normalizeStringifyOptions(opts) {
    ...
    var arrayFormat;
    if (opts.arrayFormat in arrayPrefixGenerators) arrayFormat = opts.arrayFormat;else
    if ("indices" in opts) arrayFormat = opts.indices ? "indices" : "repeat";else
  ...
  module.exports = function (object, opts) {
    ...
    if (options.charsetSentinel) {
      if (options.charset === "iso-8859-1") prefix += "utf8=%26%2310003%3B&";else
  ...
  var parseValues = function parseQueryStringValues(str, options) {
    ...
    if (options.charsetSentinel) {
      for (i = 0; i < parts.length; ++i) if (parts[i].indexOf("utf8=") === 0) {
        if (parts[i] === charsetSentinel) charset = "utf-8";else
        if (parts[i] === isoSentinel) charset = "iso-8859-1";
        skipIndex = i;
        i = parts.length;
      }
    }
    for (i = 0; i < parts.length; ++i) {
      ...
      if (existing && options.duplicates === "combine") obj[key] = utils.combine(obj[key], val);else
      if (!existing || options.duplicates === "last") obj[key] = val;
    }
    return obj;
  };
  var parseObject = function (chain, val, options, valuesParsed) {
    ...
    for (var i = chain.length - 1; i >= 0; --i) {
      ...
      if (root === "[]" && options.parseArrays) obj = options.allowEmptyArrays && (leaf === "" || options.strictNullHandling && leaf === null) ? [] : utils.combine([], leaf);else
      {
        ...
        if (!options.parseArrays && decodedRoot === "") obj = { 0: leaf };else
...
//#endregion
//#region ../../node_modules/.pnpm/body-parser@2.2.0/node_modules/body-parser/lib/types/urlencoded.js
/*!
* body-parser
* Copyright(c) 2014 Jonathan Ong
* Copyright(c) 2014-2015 Douglas Christopher Wilson
...
var require_urlencoded = /* @__PURE__ */__commonJSMin((exports, module) => {
  ...
  * @private
  */
  var createError$2 = __require("http-errors");
  var debug = require_browser$2()("body-parser:urlencoded");
  var isFinished = require_on_finished().isFinished;
  var read = require_read();
  var typeis = require_type_is();
  var qs = require_lib();
  ...
  function createQueryParser(options) {
    ...
    return function queryparse(body, encoding) {
      ...
      try {
        ...
        if (err instanceof RangeError) throw createError$2(400, "The input exceeded the depth", { type: "querystring.parse.rangeError" });else
        throw err;
      }
    };
  }
...
//#endregion
//#region ../../node_modules/.pnpm/body-parser@2.2.0/node_modules/body-parser/index.js
/*!
* body-parser
* Copyright(c) 2014-2015 Douglas Christopher Wilson
* MIT Licensed
...
//#endregion
//#region ../../node_modules/.pnpm/merge-descriptors@2.0.0/node_modules/merge-descriptors/index.js
...
//#endregion
//#region ../../node_modules/.pnpm/finalhandler@2.1.0/node_modules/finalhandler/index.js
/*!
* finalhandler
* Copyright(c) 2014-2022 Douglas Christopher Wilson
* MIT Licensed
  ...
  function finalhandler(req, res, options) {
    ...
    return function (err) {
      ...
      if (err) {
        ...
        if (status === void 0) status = getResponseStatusCode(res);else
...
//#endregion
//#region ../../node_modules/.pnpm/express@5.1.0/node_modules/express/lib/view.js
/*!
* express
...
var require_ipaddr = /* @__PURE__ */__commonJSMin((exports, module) => {
  (function () {
    ...
    if (typeof module !== "undefined" && module !== null && module.exports) module.exports = ipaddr;else
    root["ipaddr"] = ipaddr;
    ...
    ipaddr.IPv4.parser = function (string) {
      var match,parseIntAuto = function (string) {
          if (string[0] === "0" && string[1] !== "x") return parseInt(string, 8);else
    ...
    ipaddr.IPv6 = function () {
      function IPv6(parts, zoneId) {
        ...
        } else if (parts.length === 8) this.parts = parts;else
    ...
    ipaddr.IPv6.parser = function (string) {
      ...
      if (ipv6Regexes["native"].test(string)) return expandIPv6(string, 8);else
    ...
    ipaddr.IPv4.isValidFourPartDecimal = function (string) {
      if (ipaddr.IPv4.isValid(string) && string.match(/^(0|[1-9]\d*)(\.(0|[1-9]\d*)){3}$/)) return true;else
      return false;
    };
    ...
    ipaddr.parse = function (string) {
      if (ipaddr.IPv6.isValid(string)) return ipaddr.IPv6.parse(string);else
      if (ipaddr.IPv4.isValid(string)) return ipaddr.IPv4.parse(string);else
    ...
    ipaddr.fromByteArray = function (bytes) {
      ...
      if (length === 4) return new ipaddr.IPv4(bytes);else
      if (length === 16) return new ipaddr.IPv6(bytes);else
      throw new Error("ipaddr: the binary input is neither an IPv6 nor IPv4 address");
    };
    ipaddr.process = function (string) {
      var addr = this.parse(string);
      if (addr.kind() === "ipv6" && addr.isIPv4MappedAddress()) return addr.toIPv4Address();else
      return addr;
    };
  }).call(exports);
});
...
var require_proxy_addr = /* @__PURE__ */__commonJSMin((exports, module) => {
  ...
  */
  function compile(val) {
    ...
    var trust;
    if (typeof val === "string") trust = [val];else
    if (Array.isArray(val)) trust = val.slice();else
  ...
  */
  function parseipNotation(note) {
    ...
    if (range === null) range = max;else
    if (DIGIT_REGEXP.test(range)) range = parseInt(range, 10);else
    if (ip.kind() === "ipv4" && isip(range)) range = parseNetmask(range);else
    range = null;
...
//#endregion
//#region ../../node_modules/.pnpm/express@5.1.0/node_modules/express/lib/utils.js
/*!
* express
* Copyright(c) 2009-2013 TJ Holowaychuk
* Copyright(c) 2014-2015 Douglas Christopher Wilson
  ...
  */
  function acceptParams(str) {
    ...
    while (index < length) {
      ...
      if (key === "q") ret.quality = parseFloat(value);else
...
//#endregion
//#region ../../node_modules/.pnpm/is-promise@4.0.0/node_modules/is-promise/index.js
  ...
  function sequenceToRegExp(tokens, delimiter, keys) {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      if (token.type === "param" || token.type === "wildcard") {
        ...
        if (token.type === "param") result += `(${negate(delimiter, isSafeSegmentParam ? "" : backtrack)}+)`;else
        result += `([\\s\\S]+)`;
        keys.push(token);
        backtrack = "";
...
//#endregion
//#region ../../node_modules/.pnpm/router@2.2.0/node_modules/router/lib/layer.js
/*!
* router
* Copyright(c) 2013 Roman Shtylman
* Copyright(c) 2014-2022 Douglas Christopher Wilson
...
var require_layer = /* @__PURE__ */__commonJSMin((exports, module) => {
  ...
  */
  ...
  const debug = require_browser$1()("router:layer");
  const deprecate = require_browser()("router");
  /**
  * Module variables.
  * @private
  */
...
//#endregion
//#region ../../node_modules/.pnpm/router@2.2.0/node_modules/router/lib/route.js
/*!
* router
* Copyright(c) 2013 Roman Shtylman
* Copyright(c) 2014-2022 Douglas Christopher Wilson
...
var require_route = /* @__PURE__ */__commonJSMin((exports, module) => {
  ...
  * @private
  */
  const debug = require_browser$1()("router:route");
  ...
  */
  Route.prototype.dispatch = function dispatch(req, res, done) {
    ...
    function next(err) {
      ...
      if (err) layer.handleError(err, req, res, next);else
...
//#endregion
//#region ../../node_modules/.pnpm/router@2.2.0/node_modules/router/index.js
/*!
* router
* Copyright(c) 2013 Roman Shtylman
* Copyright(c) 2014-2022 Douglas Christopher Wilson
...
var require_router = /* @__PURE__ */__commonJSMin((exports, module) => {
  ...
  */
  ...
  const debug = require_browser$1()("router");
  const deprecate = require_browser()("router");
  /**
  * Module variables.
  * @private
  ...
  Router.prototype.handle = function handle(req, res, callback) {
    ...
    function next(err) {
      ...
      processParams(self.params, layer, paramcalled, req, res, function (err) {
        if (err) next(layerError || err);else
        if (route) layer.handleRequest(req, res, next);else
    ...
    function trimPrefix(layer, layerError, layerPath, path) {
      ...
      if (layerError) layer.handleError(layerError, req, res, next);else
      layer.handleRequest(req, res, next);
    }
  };
  /**
...
//#endregion
//#region ../../node_modules/.pnpm/express@5.1.0/node_modules/express/lib/application.js
/*!
* express
* Copyright(c) 2009-2013 TJ Holowaychuk
* Copyright(c) 2013 Roman Shtylman
...
var require_application = /* @__PURE__ */__commonJSMin((exports, module) => {
  ...
  */
  ...
  var http$3 = __require("node:http");
...
var require_charset = /* @__PURE__ */__commonJSMin((exports, module) => {
  ...
  */
  function specify(charset, spec, index) {
    var s = 0;
    if (spec.charset.toLowerCase() === charset.toLowerCase()) s |= 1;else
    if (spec.charset !== "*") return null;
    return {
      i: index,
      o: spec.i,
      q: spec.q,
...
var require_encoding = /* @__PURE__ */__commonJSMin((exports, module) => {
  ...
  */
  function specify(encoding, spec, index) {
    var s = 0;
    if (spec.encoding.toLowerCase() === encoding.toLowerCase()) s |= 1;else
...
var require_language = /* @__PURE__ */__commonJSMin((exports, module) => {
  ...
  */
  function specify(language, spec, index) {
    ...
    if (!p) return null;
    var s = 0;
    if (spec.full.toLowerCase() === p.full.toLowerCase()) s |= 4;else
    if (spec.prefix.toLowerCase() === p.full.toLowerCase()) s |= 2;else
    if (spec.full.toLowerCase() === p.prefix.toLowerCase()) s |= 1;else
...
var require_mediaType = /* @__PURE__ */__commonJSMin((exports, module) => {
  ...
  */
  function specify(type, spec, index) {
    ...
    var s = 0;
    if (!p) return null;
    if (spec.type.toLowerCase() == p.type.toLowerCase()) s |= 4;else
    if (spec.type != "*") return null;
    if (spec.subtype.toLowerCase() == p.subtype.toLowerCase()) s |= 2;else
    if (spec.subtype != "*") return null;
  ...
  */
  function splitMediaTypes(accept) {
    var accepts = accept.split(",");
    for (var i = 1, j = 0; i < accepts.length; i++) if (quoteCount(accepts[j]) % 2 == 0) accepts[++j] = accepts[i];else
    accepts[j] += "," + accepts[i];
    accepts.length = j + 1;
    return accepts;
  }
  ...
  */
  function splitParameters(str) {
    ...
    for (var i = 1, j = 0; i < parameters.length; i++) if (quoteCount(parameters[j]) % 2 == 0) parameters[++j] = parameters[i];else
...
//#endregion
//#region ../../node_modules/.pnpm/accepts@2.0.0/node_modules/accepts/index.js
/*!
* accepts
* Copyright(c) 2014 Jonathan Ong
* Copyright(c) 2015 Douglas Christopher Wilson
  ...
  */
  function combineRanges(ranges) {
    ...
    for (var j = 0, i = 1; i < ordered.length; i++) {
      ...
      if (range.start > current.end + 1) ordered[++j] = range;else
...
//#endregion
//#region ../../node_modules/.pnpm/express@5.1.0/node_modules/express/lib/request.js
/*!
* express
* Copyright(c) 2009-2013 TJ Holowaychuk
* Copyright(c) 2013 Roman Shtylman
...
var require_request = /* @__PURE__ */__commonJSMin((exports, module) => {
  ...
  */
  defineGetter(req, "host", function host() {
    ...
    if (!val || !trust(this.connection.remoteAddress, 0)) val = this.get("Host");else
    if (val.indexOf(",") !== -1) val = val.substring(0, val.indexOf(",")).trimRight();
    return val || void 0;
  });
  /**
...
//#endregion
//#region ../../node_modules/.pnpm/content-disposition@1.0.0/node_modules/content-disposition/index.js
/*!
* content-disposition
* Copyright(c) 2014-2017 Douglas Christopher Wilson
* MIT Licensed
...
//#endregion
//#region ../../node_modules/.pnpm/cookie-signature@1.2.2/node_modules/cookie-signature/index.js
...
var require_send = /* @__PURE__ */__commonJSMin((exports, module) => {
  ...
  * @private
  */
  var createError$1 = __require("http-errors");
  ...
  */
  function createHttpError(status, err) {
    if (!err) return createError$1(status);
...
//#endregion
//#region ../../node_modules/.pnpm/express@5.1.0/node_modules/express/lib/response.js
/*!
* express
* Copyright(c) 2009-2013 TJ Holowaychuk
* Copyright(c) 2014-2015 Douglas Christopher Wilson
...
var require_response = /* @__PURE__ */__commonJSMin((exports, module) => {
  ...
  */
  res.send = function send(body) {
    ...
    switch (typeof chunk) {
      ...
      case "boolean":
      case "number":
      case "object":if (chunk === null) chunk = "";else
    ...
    var len;
    if (chunk !== void 0) {
      if (Buffer.isBuffer(chunk)) len = chunk.length;else
      if (!generateETag && chunk.length < 1e3) len = Buffer.byteLength(chunk, encoding);else
      {
    ...
    if (req.method === "HEAD") this.end();else
  ...
  */
  res.jsonp = function jsonp(obj) {
    ...
    if (typeof callback === "string" && callback.length !== 0) {
      ...
      if (typeof body === "string") body = body.replace(/\u2028/g, "\\u2028").replace(/\u2029/g, "\\u2029");
  ...
  */
  res.format = function (obj) {
    ...
    } else if (obj.default) obj.default(req, this, next);else
    next(createError(406, { types: normalizeTypes(keys).map(function (o) {
        return o.value;
      }) }));
    return this;
  };
  ...
  */
  res.redirect = function redirect(url) {
    ...
    if (this.req.method === "HEAD") this.end();else
    this.end(body);
  };
  /**
...
//#endregion
//#region ../../node_modules/.pnpm/express@5.1.0/node_modules/express/lib/express.js
/*!
* express
* Copyright(c) 2009-2013 TJ Holowaychuk
* Copyright(c) 2013 Roman Shtylman
...
//#endregion
//#region ../../node_modules/.pnpm/express@5.1.0/node_modules/express/index.js
/*!
* express
* Copyright(c) 2009-2013 TJ Holowaychuk
* Copyright(c) 2013 Roman Shtylman
...
const parseParamsHash = (u) => {
  ...
  let i = 0;
  const splited = [];
  for (; i < matchs.length; i++) splited.push(u.slice(matchs[i - 1]?.index ?? 0, matchs[i].index));
...
const mergePath = (...paths) => {
  ...
  const path = paths.map((v, i, a) => {
    if (i !== 0) v = v.replace(/^\//, "");
    if (i !== a.length - 1) v = v.replace(/\/$/, "");
    return v;
  }).filter(Boolean).join("/").replace(/\/$/, "");
  return path.startsWith("/") ? path : "/" + path;
};
const mergeParams = (...ps) => {
  const res = new URLSearchParams();
  if (ps.length === 0) return res;
  for (let p of ps) {
    p = new URLSearchParams(p);
    for (const [k, v] of p) {
      const has = res.get(k);
      if (has != null) res.set(k, has.concat(",", v));else
      res.set(k, v);
    }
  }
  return res;
...
function createProxyMiddleware(options) {
  return (req, res, next) => {
    if (options.bypass) {
      if (options.bypass(req) === false) return next();
    }
    const targetUrl = new URL(options.target);
    const parseOriginUrl = parseParamsHash(req.originalUrl);
    let rewrittenPath = parseOriginUrl.origin;
    if (options.pathRewrite) {
      if (typeof options.pathRewrite === "function") rewrittenPath = options.pathRewrite(rewrittenPath);else
    ...
    if (parseTarget.origin.endsWith("/")) rewrittenPath = parseUrl.origin;
    ...
    const requestOptions = {
      ...
      path: rewrittenPath + (mergedParams.size ? `?${mergedParams.toString()}` : "") + hash,
      method: req.method,
      headers: { ...req.headers }
    };
    ...
    const proxyReq = (targetUrl.protocol === "https:" ? https : http).request(requestOptions, (proxyRes) => {
      options.onProxyReq?.(Object.assign({}, options, {
        path: requestOptions.path ?? "/",
        url: `${targetUrl.origin.replace(/\/$/, "")}/${(requestOptions.path ?? "/").replace(/^\//, "")}`.replace(/\/$/, "")
      }), req, res);
...
//#endregion
//#region ../http/src/http/print-proxy.ts
...
*/
function generateAlignedProxyReport(proxyRules, detail = false) {
  ...
  const maxPathLength = Math.max(...Object.keys(proxyRules).map((path) => {
    return (path.endsWith("/") ? path : `${path}/`).length;
  }));
  return Object.entries(proxyRules).map(([path, rule]) => {
    ...
    if (detail && typeof rule !== "string") {
      ...
      if (rule.pathRewrite) {
        if (typeof rule.pathRewrite === "function") options.push(`pathRewrite: ${rule.pathRewrite.toString()}`);else
...
* @param proxyRules 代理规则配置
*/
function printAlignedProxyServerInfo(port, proxyRules, printer = console.log.bind(console), detail = false) {
...
//#endregion
//#region ../http/src/http/createServer.ts
/**
*
* @param port
* @param option
...
function createServer(port, option = {}) {
  ...
  const defaultMiddles = [(0, import_lib.default)({ origin: "*" }), import_express.json()];
  if (middles && !isArray(middles) && typeof middles !== "function") {
    const m = middles.define;
    [m(defaultMiddles)].flat().reduce((a, b) => a.use(b), server);
  } else [...defaultMiddles, middles || []].flat().reduce((a, b) => a.use(b), server);
  if (routes) {
    const parseRoute = (routes, parentPath) => {
      for (const route of routes) {
        let { path, children, method, handlers = [] } = route;
        path = parentPath + path;
        if (method) server[method].bind(server)(path, ...handlers);
  ...
  server.listen(port, () => {
    ...
    const printProxy = isObject(option.printProxy) ? option.printProxy : {
      print: option.printProxy,
      detail: false
    };
    printProxy.print !== false && printAlignedProxyServerInfo(port, proxy, void 0, printProxy.detail);
  });
  return server;
}
function staticMiddle(path) {
  return import_express.static(path);
}
//#endregion
...
var Logger = class {
  ...
  constructor(prefix) {
    this.prefix = prefix;
    Object.defineProperties(this, Object.keys(colorMap).reduce((map, key) => {
      map[key] = { value: function (message, ...args) {
  ...
  log(level, message, ...args) {
    let prefix = `[${(/* @__PURE__ */new Date()).toLocaleTimeString()}] ${this.prefix}:`;
...
var PluginError = class extends Error {
  constructor(pluginName, message, errorOption) {
    super(`[xbuild-plugin:${pluginName}]: ${message}`, errorOption);
  }
};
var PluginManager = class PluginManager {
  constructor(plugins) {
    if (plugins instanceof PluginManager) return plugins;else
  ...
  get sortPlugins() {
    const notOrder = this.plugins.filter(({ order }) => typeof order === "undefined");
    return [...this.plugins.filter(({ order }) => typeof order !== "undefined").sort((a, b) => a.order - b.order), ...notOrder];
  }
  checkPluginName() {
    this.pluginMap.clear();
    try {
      for (const plugin of this.plugins) {
        if (this.pluginMap.has(plugin.name)) throw new PluginError(plugin.name, "the name of plugin is exist.");
  ...
  transform(code, id, source) {
    ...
    try {
      ...
      throw new PluginError(pluginName, e instanceof Error ? e.message : String(e));
    }
  }
  apply(hook, ...args) {
...
//#endregion
//#region src/core/htmlEntryPlugin.ts
const htmlEntryPlugin = () => {
  ...
  return {
    ...
    async buildStart(options) {
      ...
      if (typeof input === "object" || !input.endsWith(".html")) return;
      const htmlPath = path.resolve(process.cwd(), input);
      console.log(htmlPath);
      htmlContent = fs.readFileSync(htmlPath, "utf-8");
      const scriptRegex = /<script\s+[^>]*src\s*=\s*['"]([^'"]+\.(js|ts|jsx|tsx))['"][^>]*>/gi;
      const scripts = [];
      let match;
      ...
      if (scripts.length > 0) {
        ...
        for (let i = 1; i < scripts.length; i++) this.emitFile({
          type: "chunk",
          id: scripts[i]
        });
      }
    ...
    async generateBundle(outputOptions, bundle) {
      ...
      for (const [fileName, assetInfo] of Object.entries(bundle)) {
        ...
        if (ext === ".css") cssLinks.push(`<link rel="stylesheet" href="${fileName}">`);else
        if (ext === ".js" && assetInfo.type === "chunk" && !assetInfo.isEntry) jsScripts.push(`<script src="${fileName}" defer><\/script>`);else
        if (assetTypeMap[ext]) headInjection.push(`<link rel="preload" href="${fileName}" as="${assetTypeMap[ext]}" crossorigin>`);
      }
      let updatedHtml = htmlContent;
      updatedHtml = updatedHtml.replace(/<script\s+[^>]*src\s*=\s*['"]([^'"]+\.(js|ts|jsx|tsx))['"][^>]*>/gi, (match, src) => {
        const outputFile = Object.keys(bundle).find((name) => name.startsWith(path.basename(src, path.extname(src))));
        return outputFile ? match.replace(src, outputFile).replace(/(type\s*=\s*['"])[^'"]*['"]/i, "type=\"module\"") : match;
      });
      updatedHtml = updatedHtml.replace(/<\/head>/i, `${cssLinks.join("\n")}\n${headInjection.join("\n")}\n</head>`);
      updatedHtml = updatedHtml.replace(/<\/body>/i, `${jsScripts.join("\n")}\n</body>`);
      this.emitFile({
        type: "asset",
        fileName: "index.html",
        source: updatedHtml
...
const defaultRolldownPlugins = [postcss({
  extract: true,
  sourceMap: true
}), htmlEntryPlugin()];
/**
...
var XBuilder = class {
  ...
  buildHtml(filePath) {
    const fileContent = fs$1.readFileSync(path.resolve(filePath), "utf-8");
    const html = new JSDOM(fileContent);
    const script = html.window.document.querySelector("script#script-main[type=module]");
    if (!script) return filePath;else
    {
      const src = script.src;
      const changeString = (str, splitor, replacer) => {
        const _replacer = typeof replacer === "function" ? replacer : () => replacer;
        return str.split(splitor).map((v, i, a) => i === a.length - 1 ? _replacer(v) : v).join(splitor);
      };
      script.src = "./" + changeString(src.split("/").pop(), "/", (v) => changeString(v, ".", "js"));
  ...
  async generate(bundle, output) {
    ...
    if (chunk.length > 0) {
      ...
      if (res && typeof res === "object") chunk[0] = {
        ...chunk[0],
        ...res
      };
    }
  ...
  get rolldownOptions() {
    ...
    };else
    {
      ...
      options.output = [options.output].flat().filter(Boolean).map((out) => {
        const result = { ...out };
        if (typeof out.chunkFileNames === "function") {
          const fn = out.chunkFileNames;
          result.chunkFileNames = (info) => fn(info, out.format || "esm") || info.name;
        }
        return result;
      });
      options.plugins = [...defaultRolldownPlugins].concat([config.build?.plugins].flat().filter(Boolean));
      options.external = [...defaultRolldownExternal, ...[config.build.external].flat()];
      return options;
    }
  }
  write(filePath, data, options) {
    let _options = options;
    if (typeof options === "string") _options = { encoding: options };else
    if (options === null) _options = {};
    const dirpath = path.dirname(filePath);
    if (!fs$1.existsSync(dirpath)) fs$1.mkdirSync(dirpath, { recursive: true });
    return new Promise((resolve, reject) => {
      fs$1.writeFile(filePath, data, {
        encoding: "utf-8",
        ..._options
      }, (error) => {
        if (!error) resolve();else
        reject(error);
      });
      if (_options.signal) _options.signal.addEventListener("abort", () => {
        if (fs$1.existsSync(dirpath)) fs$1.rmSync(dirpath, { recursive: true });
      });
    });
  }
  async runBuild() {
    try {
      ...
      if (options.input) {
        const inputs = [];
        for (const input of [options.input].flat()) if (!input.endsWith(".html")) inputs.push(input);else
      ...
      for (const input of inputs) await this.buildOne(input);
      this.logger.success("Build completed successfully");
      return true;
    } catch (error) {
  ...
  async buildOne(input) {
    const { input: _, output, ...options } = this.rolldownOptions;
    const bundle = await rolldown({
      ...options,
      input
    });
    const promiseResults = [output].flat().filter(Boolean).map(async (output) => {
      const bundled = await this.generate(bundle, output);
      return {
        output,
        chunks: bundled.filter((v) => v.type === "chunk"),
        assets: bundled.filter((v) => v.type === "asset")
      };
    });
    ...
    const write = (writePath, content) => {
      writes.push(this.write(writePath, content, {
        encoding: "utf-8",
    ...
    for (const { output, chunks, assets } of result) {
      ...
      chunks.forEach(({ fileName, facadeModuleId, code, sourcemapFileName, map, ...option }) => {
        fileName = nil(typeof chunkFileNames === "function" ? chunkFileNames({
          ...option,
      ...
      createServer(option.port, {
        ...
        routes: [{
          ...
          handlers: [async (request, response) => {
            const { modulePath } = request.query;
            if (!modulePath) {
            ...
            if (!fs$1.existsSync(options.input)) {
              response.sendStatus(404);
              return;
            }
            const bundleCode = (await (await rolldown(options)).generate({
              format: "esm",
              file: "index.js",
              sourcemap: false
            })).output[0].code;
            ...
            handlers: [(request, response) => {
              ...
              */
              function buildFileTree(rootPath) {
                if (!fs$1.existsSync(rootPath)) return [];
                function buildTree(currentDir, relativePath) {
                  const items = fs$1.readdirSync(currentDir, { withFileTypes: true });
                  const dirs = [];
                  const files = [];
                  for (const item of items) {
                    ...
                    if (item.isDirectory()) {
                      ...
                      if (children.length > 0) dirs.push({
                        name: item.name,
                        path: itemRelativePath,
                        type: "directory",
                        children
                      });
                    } else if (item.isFile() && /.*\.(demo|dev)\.tsx?$/.test(item.name)) files.push({
                      name: item.name,
                      path: itemRelativePath,
                      type: "file"
                    });
...
let inst;
const _XbuildEnv = new new Proxy(XBuildENV, { construct(target, argArray, newTarget) {
    if (!inst) inst = new XBuildENV(argArray[0]);
    return inst;
  } })(XBUILD_ENV);
//#endregion
...
function getDefault(d) {
  ...
  if (_default) return _default;else
  return d;
}
  ...
  return {
    ...
    transform(code, id) {
      try {
        ...
        getDefault(traverse)(ast, { Program(path) {
            ...
            programBody.forEach((node) => {
              if (types.isExpressionStatement(node)) {
                ...
                if (types.isCallExpression(expression)) {
                  ...
                  if (types.isIdentifier(callee) && callee.name === "__decorate") {
...
async function loadConfig(userConfigPath, options) {
  const configPath = findConfigFile(userConfigPath);
  const finalConfig = { pluginManager: new PluginManager() };
  if (!configPath) {
  ...
  try {
    ...
    const userConfigModule = await import(toFileUrl(path.resolve(tmpConfigPath))).finally(() => {
      fs.rmSync(path.resolve(tmpConfigPath), { recursive: true });
    });
    const userConfig = userConfigModule.default || userConfigModule;
    const resolvedConfig = await Promise.resolve(typeof userConfig === "function" ? userConfig({ mode: _XbuildEnv.env || "production" }) : userConfig);
    Object.assign(finalConfig, resolvedConfig);
    finalConfig.pluginManager.addPlugins(resolvedConfig.plugins?.filter(({ name }) => {
      if (!options?.compile) return true;else
      return name !== "typescript";
    }));
    finalConfig.pluginManager.addPlugin(__decoratorHandle());
    if (finalConfig.build?.plugins) {
      let plugins = finalConfig.build.plugins;
      if (isPromiseLike(plugins)) plugins = await plugins;
      const pluginsHandle = (plugins) => {
        if (!plugins) return plugins;else
        if (Array.isArray(plugins)) return plugins.filter(pluginsHandle);else
        if (typeof plugins === "object" && plugins !== null) {
          if (options?.compile) return plugins["name"] === "typescript" ? false : plugins;else
          return plugins;
        }
      };
...
function findConfigFile(userPath) {
  ...
  for (const configPath of possiblePaths) {
    ...
    const withJsExt = absolutePath.endsWith(".js") ? absolutePath : `${absolutePath}.js`;
...
async function buildCommand(options) {
  ...
  _XbuildEnv.to("production");
  try {
    const isCompile = options.compile === true;
    const _config = {
      ...(await loadConfig(options.config, { compile: isCompile })),
      mode: "production"
    };
    logger.info(`Starting ${isCompile ? "compile" : "build"} of full build process...`);
...
async function devCommand(options) {
  ...
  _XbuildEnv.to("development");
  try {
    const _config = {
      ...(await loadConfig(options.config, { compile: true })),
      mode: "development"
    };
    logger.info("Starting development server...");
    new XBuilder(_config).runDev(options).then(({ port, proxy }) => {
      logger.success(`Development server running at http://localhost:${port}`);
      proxy && printAlignedProxyServerInfo(port, proxy, logger.success.bind(logger));
    }).catch((error) => {
...
//#endregion
//#region src/index.ts
program.name("xbuild").description("Extensible Rollup-based TypeScript build tool").version("0.1.0");
program.command("dev").description("Start development server").option("-s, --static <path>", "Path to public dir").option("-c, --config <path>", "Path to config file").option("-p, --port <number>", "Path to Port number, default 9999").action(async (options) => {
  await devCommand({
    ...options,
    public: options.static
  });
});
program.command("build").description("Full build process with type checking and declaration files").option("-c, --config <path>", "Path to config file").option("-C, --compile", "Path to compile mode").action(async (options) => {
...
//#endregion
export { Logger, PluginManager, XBuilder, buildCommand, defineConfig, devCommand, loadConfig };