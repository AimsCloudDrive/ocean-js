...
export interface OcPromiseLike<R = any, E = any, C = any> extends PromiseLike<R, E> {
  ...
  ): OcPromiseLike<InferResultR<TR, TE, TC>, InferResultE<TR, TE, TC>, InferResultC<TR, TE, TC>>;
...
export type OcPromiseExecutor<R, E extends Error | unknown = Error, C = unknown> = createFunction<
  [Resolve<R>, Reject<E>, Cancel<C>, void]
>;

export type InferResultR<TR = never, TE = never, TC = never> =
  | (TR extends PromiseLike<infer R, any> ? R : TR extends Promise<infer R> ? R : TR)
  | (TE extends PromiseLike<infer R, any> ? R : TE extends Promise<infer R> ? R : TE)
  | (TC extends PromiseLike<infer R, any> ? R : TC extends Promise<infer R> ? R : TC);

export type InferResultE<TR = never, TE = never, TC = never> =
  | (TR extends PromiseLike<any, infer E> ? E : TR extends Promise<any> ? unknown : never)
  | (TE extends PromiseLike<any, infer E> ? E : TE extends Promise<any> ? unknown : never)
  | (TC extends PromiseLike<any, infer E> ? E : TC extends Promise<any> ? unknown : never);
