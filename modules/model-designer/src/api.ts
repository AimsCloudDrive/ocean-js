...
export function createHttpModelDesignerApi(
  ...
  return {
    ...
    updateRelation: async (id, patch: RelationPatch) =>
      toModelRelation(await request<ServerRelation>(`/relations/${encodeURIComponent(id)}`, "PATCH", relationPatchPayload(patch))),
    deleteRelation: async (id) => {
      await request(`/relations/${encodeURIComponent(id)}`, "DELETE");
    },
    setLocked: async (locked) => {
      const result = await request<ModelDesignerCanvas & { META_TYPE: "base" }>("/canvas", "PUT", { ...canvas, locked });
